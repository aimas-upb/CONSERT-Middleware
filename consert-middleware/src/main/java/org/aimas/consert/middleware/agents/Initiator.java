...
public class Initiator extends AbstractVerticle {
	...
	public static final String ADDRESS = "127.0.0.1"; // Address where other
														...
														// server
	public static final int LISTENING_PORT = 8080; // Port where the server is
													// listening
	...
	private final int PARTICIPANT_PORT = 8081; // Port where the participant can
												// be reached
	private final String PARTICIPANT_HOST = "127.0.0.1"; // Address where the
															// participant can
															// be reached
	private final String REQUEST_LONG_URI = "/user_long_query"; // URI to use on
																// participant
																// to make a
																// long query
	private final String REQUEST_SHORT_URI = "/user_short_query"; // URI to use
																	// on
																	// participant
																	// to make a
																	// short
																	// query
	private final String CALLBACK_URI = "/query_result"; // URI on this server
															// for the
															// participant's
															// callback
	...
	public void start() {
		...
		// Start server
		Initiator.vertx.createHttpServer().requestHandler(router::accept).listen(LISTENING_PORT, HOST, res -> {
			if (res.succeeded()) {
	...
	private void handleMakeQuery(RoutingContext rtCtx) {
		...
		// Receive the URI of the dynamically created route if the query is
		// long, or the result otherwise
			...
			public void handle(HttpClientResponse resp) {
				...
				resp.bodyHandler(new Handler<Buffer>() {
					...
					public void handle(Buffer buffer) {
						...
						if (statusCode == 200) { // Long query, get resource URI
													// to read result later
							System.out.println("Long query, waiting for result in resource at " + buffer.toString());
						} else if (statusCode == 201) { // Short query, get
														// result now
		...
		}).putHeader("content-type", "application/json").end(Json.encodePrettily(reqBean));
