...
/**
 * Server simulating a participant
 */
public class Participant extends AbstractVerticle {

	public static final String ADDRESS = "127.0.0.1"; // Address where other
														// agents can reach this
														// server
	public static final int LISTENING_PORT = 8081; // Port where the server is
													// listening
	public static final String HOST = "0.0.0.0"; // Host name that can be used
													// to reach this server
													// (0.0.0.0 = any)

	private static Vertx vertx = Vertx.vertx(); // Vertx instance

	private Map<UUID, RequestResource> resources; // List of reachable
	...
	public void start() {
		...
		// Start server
		Participant.vertx.createHttpServer().requestHandler(router::accept).listen(LISTENING_PORT, HOST, res -> {
			if (res.succeeded()) {
	...
	private void handleUserLongQuery(RoutingContext rtCtx) {
			...
			public void handle(HttpClientResponse resp) {
				resp.bodyHandler(new Handler<Buffer>() {
					...
					public void handle(Buffer buffer) {
						...
						rtCtx.response().putHeader("content-type", "text/plain").setStatusCode(200)
						...
						client.put(Participant.LISTENING_PORT, "127.0.0.1", routeUUID, ignoreResponseHandler)
								.putHeader("content-type", "text/plain").end("state=agree_sent");

						// Get result
						String result = "the result";

						client.put(Participant.LISTENING_PORT, "127.0.0.1", routeUUID, ignoreResponseHandler)
								.putHeader("content-type", "text/plain").end("result=" + result);

						// Extract the information from the initiator callback
						// URI
						...
						client.put(Participant.LISTENING_PORT, "127.0.0.1", routeUUID, ignoreResponseHandler)
								.putHeader("content-type", "text/plain").end("state=result_sent");
		...
		}).putHeader("content-type", "application/json").end(rtCtx.getBodyAsString());
	}
	...
	private void handleUserShortQuery(RoutingContext rtCtx) {
		...
		rtCtx.response().putHeader("content-type", "text/plain").setStatusCode(201).end(result);
	}
	...
	private void handleGetAllResources(RoutingContext rtCtx) {
		...
		// Send resources
		rtCtx.response().putHeader("content-type", "application/json; charset=utf-8").setStatusCode(200)
	...
	private void handleUpdateResource(RoutingContext rtCtx) {
		...
		if (resource != null) {
			...
			if (body.equals("state=agree_sent")) {
				...
				System.out.println(
						"update resource " + resourceUUID + ": result = " + body.substring("result=".length()));
			} else {
				updateOk = false;
